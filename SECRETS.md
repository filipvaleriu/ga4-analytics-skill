...
python scripts/secret_store.py migrate ga4 config/ga4.ini
#   -> muta client_secret + refresh_token in Credential Manager si le goleste in .ini

# Varianta B: introducere manuala (valoarea e ascunsa la tastare)
python scripts/secret_store.py set ga4 client_secret
python scripts/secret_store.py set ga4 refresh_token

# Verificare (afiseaza doar prezent/lipsa, NU valorile)
python scripts/secret_store.py status ga4
```
